...
- Agent 不读取浏览器 Cookie；若已登录 Chrome 能加载目标媒体，可把页面媒体保存到受限的 `data/browser-capture`，再由容器内 CLI 安全导入同一任务。
...
├── secrets/app.key
├── cache/                 # 容器模型与解析缓存
├── browser-capture/       # Agent 从登录态页面临时保存的待导入媒体
└── downloads/
    └── <aweme_id>/
        ├── video.mp4
        ├── metadata.json
...
- Chrome 辅助导入只接受 `data/browser-capture` 内的非空本机文件，通过规范化抖音 URL 生成固定输出路径；必须在容器内执行，避免跨运行时绝对路径漂移。
